...
def listar_anectodas(id: int, token: Annotated[str, Depends(oauth2_scheme)], db: Session = Depends(get_db)):
    return service.listar_anecdotas_reseñas(id=id, db=db)

@router.post('', response_model=schemas.Anecdota)
def crear_anecdota(anecdota: schemas.AnecdotaCrear, token: Annotated[str, Depends(oauth2_scheme)], db: Session = Depends(get_db)):
    return service.crear_anecdota(db=db, anecdota=anecdota)

@router.get('/{id}', response_model=schemas.Anecdota)
def buscar_anecdota(id : int, token: Annotated[str, Depends(oauth2_scheme)], db: Session = Depends(get_db)): 
    return service.buscar_anecdota(db=db, id=id)

@router.put('/{id}', response_model=schemas.Anecdota)
def modificar_anecdota(id : int, anecdota: schemas.AnecdotaCrear, token: Annotated[str, Depends(oauth2_scheme)], db: Session = Depends(get_db)): 
    return service.modificar_anecdota(db=db, id=id, anecdota=anecdota)

@router.delete('/{id}', response_model=schemas.Anecdota)
def eliminar_anecdota(id : int, token: Annotated[str, Depends(oauth2_scheme)], db: Session = Depends(get_db)): 
