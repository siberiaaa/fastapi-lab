...
def get_tipos_productos(db: Session = Depends(get_db)):
...
def get_tipo_producto(id: int, db: Session = Depends(get_db)): 
    return service.get_tipo_producto(db=db, id=id)

@router.put('/{id}', response_model=Respuesta[schemas.Tipo_Producto])
def update_tipo_producto(id: int, tipo_producto: schemas.Tipo_ProductoCrear, db: Session = Depends(get_db)): 
